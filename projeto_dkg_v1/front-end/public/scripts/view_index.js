...
const BASE_URL = "https://rafael-front.sa.ngrok.io/search?q=";



const historyCache = JSON.parse(localStorage.getItem('clone-google-history')) || []

const userMenu = document.getElementById("user__menu")
...
function handleHistory() {
  const listHistory = document.getElementById('history');
  listHistory.innerHTML = ''; // Limpa a lista antes de popular

  const fragment = document.createDocumentFragment();

  historyCache.forEach(historyItem => {
    const li = document.createElement('li');

    const anchor = document.createElement('a');
    anchor.setAttribute('href', BASE_URL.concat(historyItem));
    anchor.classList.add('history-link'); // Classe opcional para estilização

    // Criação mais clara do conteúdo do link
    anchor.innerHTML = `
      ${ICON_CLOCK}
  ...
  listHistory.appendChild(fragment); // Insere o fragmento no DOM
}
...
function handleVisibleMenuUser(){
  menuApp.classList.remove("active")
